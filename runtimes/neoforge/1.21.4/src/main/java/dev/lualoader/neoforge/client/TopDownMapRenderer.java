...
public final class TopDownMapRenderer {
    ...
    public static void draw(GuiGraphics graphics, ScreenModel.Element element, int x, int y) {
        ...
        graphics.blit(RenderType::guiTextured, textureId, x, y, 0.0f, 0.0f,
                Math.max(1, element.w()), Math.max(1, element.h()), size, size, size, size);
    }
            ...
            for (int pixelX = 0; pixelX < state.size; pixelX++) {
                ...
                if (color == -1) {
                    ...
                    color = mapColor == MapColor.NONE
                            ? 0x30343A : mapColor.calculateARGBColor(MapColor.Brightness.NORMAL) & 0xFFFFFF;
                }
                ...
                state.image.setPixel(pixelX, pixelY, color | 0xFF000000);
            }
        }
