...
public class NeoForgeEntityGameTest {
    ...
    private static EntityType<?> requireType() {
        EntityType<?> type = BuiltInRegistries.ENTITY_TYPE.getOptional(GUARDIAO).orElse(null);
    ...
    public static void nasceComOsAtributosDeclarados(GameTestHelper helper) {
        ...
        Entity entity = type.spawn(helper.getLevel(), position, EntitySpawnReason.COMMAND);
        if (entity == null) {
    ...
    public static void ovoDeCriacaoExisteComoItem(GameTestHelper helper) {
        ...
        if (BuiltInRegistries.ITEM.getOptional(eggId).orElse(Items.AIR) == Items.AIR) {
    ...
    public static void especieHerdaDeOutroMod(GameTestHelper helper) {
        ...
        EntityType<?> type = BuiltInRegistries.ENTITY_TYPE.getOptional(eliteId).orElse(null);
        if (type == null || type == EntityType.PIG) {
    ...
    public static void especiesGeradasPorScriptExistem(GameTestHelper helper) {
        ...
        if (BuiltInRegistries.ITEM.getOptional(ResourceLocation.fromNamespaceAndPath(
                "bestiario", "guardiao_ouro_spawn_egg")).orElse(Items.AIR) == Items.AIR) {
    ...
    public static void comportamentoDeclaradoChegaNaCriatura(GameTestHelper helper) {
        BlockPos position = helper.absolutePos(new BlockPos(1, 2, 1));
        Entity entity = requireType().spawn(helper.getLevel(), position, EntitySpawnReason.COMMAND);
