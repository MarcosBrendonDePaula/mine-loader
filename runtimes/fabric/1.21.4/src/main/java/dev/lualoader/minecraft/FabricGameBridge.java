...
public final class FabricGameBridge implements GameBridge {
    ...
    public boolean damageEntity(String entityUuid, float amount) {
        ...
        var world = requireWorld();
        return entity.damage(world, world.getDamageSources().magic(), amount);
    }
    ...
    public java.util.List<String> recipesUsing(String itemId, int limit) {
        ...
        return collectRecipes(limit, recipe -> {
            for (Ingredient ingredient : recipe.getIngredientPlacement().getIngredients()) {
                if (ingredient.getMatchingItems()
                        .anyMatch(entry -> Registries.ITEM.getId(entry.value()).equals(wanted))) {
                    return true;
                }
            }
            return false;
    ...
    /** Resultado exibido pela receita, substituto do getResult removido na 1.21.4. */
    private ItemStack recipeResult(Recipe<?> recipe) {
        var context = SlotDisplayContexts.createParameters(requireWorld());
        return recipe.getDisplays().stream()
                .map(display -> display.result().getFirst(context))
    ...
    private java.util.List<String> collectRecipes(int limit,
        ...
        for (RecipeEntry<?> entry : requireServer().getRecipeManager().values()) {
            ...
            try {
                ...
                found.add(describeRecipe(entry.id().getValue(), recipe));
    ...
    private String describeRecipe(Identifier id, Recipe<?> recipe) {
        ...
        for (Ingredient ingredient : recipe.getIngredientPlacement().getIngredients()) {
            ...
            ingredient.getMatchingItems()
                    ...
                    .forEach(entry -> alternatives.add(
                            Registries.ITEM.getId(entry.value()).toString()));
