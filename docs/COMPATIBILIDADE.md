...
| `container_slot_layout(x, y, z)` -- onde cada slot fica na tela da maquina | sim | sim | Le o menu que a propria maquina declara. E o que deixa a tela de configuracao ter **a forma da maquina** em vez de uma fileira: o jogador reconhece a fornalha pelos tres slots em L. Vazio quando o bloco nao tem menu proprio, e ai quem desenha cai na fileira |
| `open_block_inventory(x, y, z)` | sim | sim | Abre a janela do proprio bloco, do jeito que o jogo abriria. O mod nao imita a tela da maquina: pede que ela se abra |
| `layer` num elemento de tela | sim | sim | Vira **profundidade**, e nao ordem de desenho: o jogo desenha icone de item com deslocamento proprio de ~100, entao a ordem da lista sozinha nao empilha painel sobre item. Vale tambem para botao e campo de texto, que sao widgets do jogo -- sem isso, um modal cobria os proprios botoes e eles sumiam de uma tela que continuava respondendo ao clique |
