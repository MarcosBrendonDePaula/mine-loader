...
git clone https://github.com/MarcosBrendonDePaula/mine-loader.git
cd mine-loader
${windowsWrapper}gradlew.bat :runtimes:fabric:1.21.4:runServer`;

const eula = `# runtimes/fabric/1.21.4/run/eula.txt
eula=true`;
    ...
    >
      <section className="doc-section doc-opening">
        ...
        <div className="doc-two-col align-start">
          <p className="doc-lead">Instale o Java 21 e tenha um editor de texto. Para o primeiro mod, vamos usar o runtime Fabric 1.21.4.</p>
          <div className="doc-copy"><p>O repositório já traz o Gradle Wrapper. Não instale Gradle, Fabric API ou NeoForge separadamente para seguir este começo.</p><p>O comando abaixo inicia um servidor de desenvolvimento e lê mods em <code>runtimes/fabric/1.21.4/run/mods-lua/</code>.</p></div>
        </div>
        <div className="first-run-map" aria-label="Fluxo da primeira execução">
          ...
          <div className="first-run-map-grid"><div><span>01</span><strong>REPOSITÓRIO</strong><p>Java 21 + Gradle Wrapper</p></div><div><span>02</span><strong>RUNTIME</strong><p>Fabric 1.21.4</p></div><div><span>03</span><strong>PASTA DO MOD</strong><p><code>run/mods-lua/meu_mod</code></p></div><div><span>04</span><strong>PROVA</strong><p>Uma linha no log</p></div></div>
        </div>
        <DocCode language="terminal">{startServer}</DocCode>
        <DocCallout title="A primeira execução para na licença" tone="warning">Ela baixa dependências, cria <code>runtimes/fabric/1.21.4/run/eula.txt</code> e encerra até você ler e aceitar a EULA do Minecraft. Não pule esta etapa: só mude o valor abaixo se você concordar com a licença.</DocCallout>
        <DocCode language="runtimes/fabric/1.21.4/run/eula.txt">{eula}</DocCode>
      </section>

      <section className="doc-section numbered-steps">
        <div className="doc-section-label"><span>01.2</span> Crie os dois arquivos</div>
        <article className="doc-step"><div className="step-no">01</div><div><h2>Crie a pasta do seu mod</h2><p>Crie <code>runtimes/fabric/1.21.4/run/mods-lua/meu_mod/</code>. O nome da pasta e o campo <code>id</code> abaixo devem falar do mesmo mod; use minúsculas, números e sublinhado no id.</p></div><FolderTree size={22} /></article>
        <article className="doc-step"><div className="step-no">02</div><div><h2>Salve o manifesto</h2><p>Crie <code>runtimes/fabric/1.21.4/run/mods-lua/meu_mod/mod.json</code> e cole o conteúdo abaixo. Ele diz qual arquivo Lua abrir e qual função chamar quando o servidor termina de iniciar.</p></div><FileCode2 size={22} /></article>
        <DocCode language="runtimes/fabric/1.21.4/run/mods-lua/meu_mod/mod.json">{firstManifest}</DocCode>
        ...
        <DocCode language="runtimes/fabric/1.21.4/run/mods-lua/meu_mod/main.lua">{firstLua}</DocCode>
      </section>

      <section className="doc-section">
        <div className="doc-section-label"><span>01.3</span> Rode e confira</div>
        <div className="doc-two-col align-start">
          <p className="doc-lead">Rode <code>./gradlew :runtimes:fabric:1.21.4:runServer</code> novamente. Agora procure uma linha com <code>Meu primeiro mod carregou.</code>.</p>
